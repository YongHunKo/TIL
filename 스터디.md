...
public class AController {	
	...
	public Object daily() {
		...
		for(var i=1; i<=31; i++) {
			ja_data.add(r.nextInt(100)+1);
			//추후 MySQL과 연동되면 여기서 일매출 데이터를 얻을 예정
		}
...
//		ja_cate.add("29일");
//		ja_cate.add("30일");
//		ja_cate.add("31일");// 복붙31일로 31일데이터가 나오는걸 확인함, for문을 사용해볼예정
		for(var i=1; i<=31; i++) {
			ja_cate.add(i+"일"); //for문 사용 완료
		}
...
public class MainController {
	...
	public String admin(Model model) {
		model.addAttribute("center","service/admin_center");
		return "main";
	}
	...
	public String loginimpl(String id, int pwd, Model model,
			HttpSession session) {
		  CustDTO cust = null;
		  try {
...
<mapper namespace="com.multi.mapper.CustMapper">
	...
	<insert id="insert" parameterType="custDTO">
		INSERT INTO cust VALUES (#{custid},#{custpwd},#{custname},#{custcredit},#{custmnum})
	</insert>
	<update id="update" parameterType="custDTO">
		UPDATE cust SET custpwd=#{custpwd}, custname=#{custname}, custcredit=#{custcredit}, custmnum=#{custmnum}  WHERE custid=#{custid}
	</update>
...
```html
<h1>LOGIN OK</h1>
<h2 th:text="${session.logincust.custname}+ ' 님 환영합니다.'"></h2>
```
### main_center
```html
<style>
...
<nav class="navbar navbar-inverse">
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="myNavbar">
      ...
      <ul th:unless="${session.logincust == null}" class="nav navbar-nav navbar-right">
      	<li th:if="${session.logincust.custid == 'aaa'}"><a href="/admin">관리자서비스</a></li>
      	<li><a href="/mypage" th:text="${session.logincust.custname}"></a></li>
  ...
  <div class="row content">
    <div class="col-sm-8">
      <div class="print_image">
      	...
      	<!-- https://iamiet.tistory.com/m/68 참고하자 -->
      	<!-- https://wasd222.blogspot.com/2020/08/html-jquery.html -->
      	<!-- 미리보기 구현 end -->
    ...
    <div class="col-sm-4 sidenav">
    	...
    	<!-- 프린터no start -->
			    <!-- <div class="form-group">
			      ...
			      <label for="printno">사용하려는 프린터 :</label>
					   <div class = "printno">
						<div class="print_1">
						  <label><input type="radio" name="printno" value="print_1" checked>프린터1</label>
						</div>
						<div class="print_2">
    	...
    	<!-- 레이아웃 start -->
			    <!-- <div class="form-group">
			      ...
			      <label for="layout">레이아웃 :</label>
					   <div class = "layout">
					    <div class="ver">
						  <label><input type="radio" name="layout" value="ver" checked>세로</label>
						</div>
						<div class="hor">
			    ...
			    <!-- <div class="form-group">
			      ...
			      <label for="print_col">컬러 :</label>
					   <div class = "col">
						...
						<div class="print_blc">
						  <label><input type="radio" name="print_col" value="blc" checked>흑백</label>
						</div>
					   </div><br>
    	<!-- 컬러 end -->
		    	...
		    	<label for="printside">단면/양면 :</label>
					   <div class = "printside">
						<div class="print_oneside">
						  <label><input type="radio" name="print_side" value="print_oneside" checked>단면</label>
						</div>
    	...
    	<!-- 개수 start -->
			    <div class="page">
			      ...
			      <input type="number" class="form-control" id="page" name="page" min="1" max="100" >
			    </div>
...
<button id = "cust">고객 현황</button>
<button id = "used_print">프린트별 사용량</button>
...
<!-- 고객 현황버튼을 누르면 MySQL과 연동했을때 cust테이블을 끌고온다 -->
<!-- 버튼 말고도 목록으로하면 편해 보이려나 -->
...
spring.datasource.url=jdbc:mysql://127.0.0.1:3306/print?serverTimezone=Asia/Seoul
...
class insertCust {
	...
	@Test
	void contextLoads() {
		CustDTO cust = new CustDTO("bbb", 222, "ko", 10000, 87654321);
		try {
...
class updateCust {
	...
	@Test
	void contextLoads() {
		CustDTO cust = new CustDTO("bbb", 222, "ko", 10000, 65432187);
		try {
			service.modify(cust);
...
- 관리자에서 금액을 추가하는 기능
- 관리자에서 매출 매입 이런거 관리
- 관리자에서 고객데이터 관리로 추후에 SELECT * FROM cust 이런거로 띄워준다 등등
- 프린트에서 미리보기 구현은 했는데 뭔가 지저분함. 스크립트 쓰는 곳이 한 곳 더 생겨버림